...
void generateTreeLt(Fss* f, ServerKeyLt* k0, ServerKeyLt* k1, uint64_t a_i, uint64_t b_i){
    ...
    int a = getBit(a_i, (64-n+1));//get the first bit of a, i.e. a_i=10010,then a=1 
    int na = a ^ 1;//na = a XOR 1

    // create arrays size (AES_key_size*2 + 2)
    unsigned char s0[32];//store the seed on the path and off the path for k0
    unsigned char s1[32];
    int aStart = 16 * a;
    int naStart = 16 *na;

    // Set initial seeds for PRF
    if(!RAND_bytes((unsigned char*) (s0 + aStart), 16)) {//生成16个随机字节，并将结果写入到 s0 数组中与 a 相关的位置
        printf("Random bytes failed\n");
        exit(1);
    }
    if (!RAND_bytes((unsigned char*) (s1 + aStart), 16)) {//生成16个随机字节，并将结果写入到 s1 数组中与 a 相关的位置
        printf("Random bytes failed\n");
        exit(1);
    }
    if (!RAND_bytes((unsigned char*) (s0 + naStart), 16)) {//生成16个随机字节，并将结果写入到 s0 数组中与 na 相关的位置
        printf("Random bytes failed\n");
        exit(1);
    }
    memcpy((unsigned char*)(s1 + naStart), (unsigned char*)(s0 + naStart), 16);//保证不在路径上时，s0和s1中的种子相同

    unsigned char t0[2];//store control bit for k0
    unsigned char t1[2];
    unsigned char temp[2];
    if (!RAND_bytes((unsigned char*) temp, 2)) {
        printf("Random bytes failed\n");
    ...
    // Figure out initial ts
    // Make sure t0a and t1a are different
    t0[a] = temp[0] % 2;//on the special path
    t1[a] = (t0[a] + 1) % 2;

    // Make sure t0na = t1na
    t0[na] = temp[1] % 2;//off the special path
    t1[na] = t0[na];
    // cout << "t0[a] t0[na]= " << t0[a] << "," << t0[na] << endl;
    // cout << "t1[a] t1[na]= " << t1[a] << "," << t1[na] << endl;
    // Set initial v's
    ...
    k1->v[na] = k0->v[na] - b_i*a; //na=0,a=1,嵌入b_i
    ...
    for (uint32_t i = 0; i < n-1; i++) {
        ...
        v = (cv[tbit0][na] + v0[na]);
        v = (v - v1[na]);
        cv[tbit1][na] = (v - b_i*a);//na=0,a=1,嵌入b_i

        // Copy appropriate values into key: cw[0][i] means the cw for left child node on i-th level on the path
        //cw[1][i] means the cw for right child node on i-th level on the path
        memcpy(k0->cw[0][i].cs[0], cs0, 16);
        memcpy(k0->cw[0][i].cs[1], (unsigned char*) (cs0 + 16), 16);
        k0->cw[0][i].ct[0] = ct0[0];
        k0->cw[0][i].ct[1] = ct0[1];
...
// Generate keys for 2 party greater than FSS 

void generateTreeGt(Fss* f, ServerKeyGt* k0, ServerKeyGt* k1, uint64_t a_i, uint64_t b_i){
    ...
    k0->v[na] = byteArr2Int64(temp_v);
    // k1->v[na] = k0->v[na] - b_i*a; //na=0,a=1,嵌入b_i <- less than gen
    k1->v[na] = k0->v[na] - b_i*(a^1);
    ...
    for (uint32_t i = 0; i < n-1; i++) {
        ...
        v = (cv[tbit0][na] + v0[na]);
        v = (v - v1[na]);
        //cv[tbit1][na] = (v - b_i*a);//na=0,a=1,嵌入b_i <- less than gen
        cv[tbit1][na] = (v - b_i*(a^1));

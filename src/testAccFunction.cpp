...
int testPlainAcc(int k, double split_ratio)
{
    ...
    for (const auto &file_name : datasets)
    {
        ...
        Dataset train, test;
        stratified_split(data, train, test, split_ratio, 42);
...
int testConstKNNAcc(int k, double split_ratio, const int num_threads)
{
    ...
    generateTreeGt(&fServer, &k0_gt2, &k1_gt2, rin, 1);
    ...
    for (const auto &file_name : datasets)
    {
        ...
        generateTreeGt(&fServer, &k0_gt, &k1_gt, n - k - 1, 1);
        ...
        double accuracy =
            static_cast<double>(correctCount) / test.data.size();
